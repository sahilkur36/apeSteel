"""Golden-file regression tests for AISC §F4.

The CSV ``flexure_F4.csv`` is generated by a stand-alone re-implementation
of the AISC 360-22 §F4 equations (no apeSteel imports).  Each row pins
every intermediate quantity (Mp, Myc, aw, rt, Rpc, FL, Lp, Lr, the three
Mn_*_Nmm values, the final Mn, phi*Mn, and the governing label) at
rel_tol = abs_tol = 1e-9.

The CSV mixes doubly-symmetric (Phase 9a) and singly-symmetric (Phase 9b)
cases; the ``geometry`` column drives which constructor is used.
"""
...
def test_F4_golden(row: dict[str, str]) -> None:
    ...
    if row["geometry"] == "DS":
        ...
        sp = ss.compute_section_properties(side)  # type: ignore[arg-type]
